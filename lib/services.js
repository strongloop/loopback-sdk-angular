 ...
 */
function generateServices(app, ngModuleName, apiUrl) {
  ngModuleName = ngModuleName || 'lbServices';
  var ngRegistryName = 'lbServicesRegistry';
  apiUrl = apiUrl || '/';
 ...
 * @header generateServices
 */
function generateRegistry(app, ngModuleName) {
  ngModuleName = ngModuleName || 'lbServicesRegistry';

  var models = describeModels(app);

  var registryTemplate = fs.readFileSync(
    require.resolve('./registry.template.ejs'),
    { encoding: 'utf-8' }
  );

  return ejs.render(registryTemplate, {
    moduleName: ngModuleName,
    models: models
  });
};
