 ...
 *
 * var client = generateServices(app, 'lbServices', '/api', clientModelConfig);
 ...
 *   server app. Default: `/`.* 
 * @param {Object} clientModelConfig Optional - The client model config that
 *   will determine what/how the services are generated. By default, (without
 *   providing any, all models wil be generated without options.
...
function applyClientModelConfig(models, clientModelConfig) {
  ...
  for (n in modelConfig) {
    ...
    for (var idx = 0; idx < sources.length; idx++) {
      var file = path.resolve(path.join(sources[idx], fsModelName + '.js'));
